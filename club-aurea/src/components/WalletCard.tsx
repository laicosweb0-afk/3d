...
import { dataBreve } from '../config/gioco';
 ...
 *
 * Dove un coupon di carta metterebbe il QR, qui ci sono le onde dell'NFC: il
 * codice resta scritto in chiaro — è quello che la cassa digita — e il
 * quadrato da inquadrare non serve più a nessuno.
 */
export function WalletCard({ nome, credito, fragranza, codice, scadenza }: Props) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative flex items-center justify-between">
        <div className="flex items-center gap-2">
          <AureaLogo size={20} />
          <span className="text-footnote font-semibold tracking-[.02em] text-white/80">Club Aurea</span>
        </div>
      ...
      <p className="relative mt-1 text-footnote text-white/55">di credito in profumeria</p>

      <div className="relative mt-6 grid grid-cols-2 gap-x-4 gap-y-4">
        <Voce etichetta="Intestato a" valore={nome} />
        <Voce etichetta="Fragranza" valore={fragranza} />
