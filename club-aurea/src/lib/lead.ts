export type Lead = {
  ...
  email: string;
  telefono: string;
  /** La fragranza che il cliente aveva nel naso mentre rispondeva. */
  fragranza: string;
  /** Le note indovinate e quante erano in tutto: dice quanto è esperto. */
  giuste: number;
  domande: number;
  credito: number;
...
export async function submitLead(lead: Lead): Promise<void> {
  ...
  if (!url) {
    ...
    console.log('[club-aurea] lead (invio simulato)', lead);
    return;
  }
